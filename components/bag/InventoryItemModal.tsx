...
export default function InventoryItemModal(props: Props) {
  ...
  function useInventoryItem() {
    ...
    if (props.item.type === ItemType.TREAT) {
      ...
      equippedItems[props.item.type] = props.item;
  ...
  return (
    <DialogContent className="sm">
      ...
      <View className="grid gap-4">
        ...
        <Text>{`Would you like to ${props.item.happiness ? "use" : "equip"} this item?`}</Text>
      </View>
      <DialogFooter>
        <DialogClose asChild>
        ...
        <Button disabled={isButtonDisabled()} onTouchEnd={useInventoryItem}>
          <Text>{`${props.item.happiness ? "Use" : "Equip"} ${props.item.name}`}</Text>
        </Button>
      </DialogFooter>
    </DialogContent>
  );
